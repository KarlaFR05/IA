...
def ucs(laberinto, inicio, meta):
    #Inicia un cronómetro para medir cuánto tarda el algoritmo
    tiempo_inicio = time.time()

    # Cola de prioridad
    cola = []
    # heapq sirve para mantener la cola ordenada por el costo acumulado, de menor a mayor
    heapq.heappush(cola, (0, inicio))

    # Guarda el menor costo conocido para llegar a cada nodo
    costos = {inicio: 0}

    # Guarda de dónde venimos para reconstruir el camino (EL NODO PADRE!!!!!!!!! Ady)
    padres = {inicio: None}

    # Nodos que ya fueron procesados
    visitados = set()

    #Usamos un while para recorrer la cola de prioridad hasta que esté vacía o encontremos la meta
    while cola:
        ...
        # Obtenemos la fila y columna del nodo actual
        fila, columna = actual

        # Izquierda, Abajo, Derecha, Arriba para explorar los vecinos en ese orden
        direcciones = [(0, -1), (1, 0), (0, 1), (-1, 0)]

        # Recorremos cada dirección para explorar los vecinos
        for df, dc in direcciones:

            # Calculamos la posición del vecino para la dirección actual y df es el cambio en fila y dc es el cambio en columna
            nueva_fila = fila + df
            
            #Calculamos la posición del vecino para la dirección actual
            ...
            if 0 <= nueva_fila < len(laberinto) and 0 <= nueva_columna < len(laberinto[0]):
                ...
                if laberinto[nueva_fila][nueva_columna] != '#':
                    ...
                    # Es para almacenar la posición del vecino en una tupla (fila, columna)
                    vecino = (nueva_fila, nueva_columna)
                    ...
                    # Solo consideramos este nuevo vecino si es la primera vez que lo vemos, o si encontramos una forma más barata de llegar 
                    # a él que la que conocíamos antes.
                    if vecino not in costos or nuevo_costo < costos[vecino]:

                        # Guardamos el nuevo costo y el padre del vecino
    ...
    if meta in padres:
        ...
        # Mientras haya un nodo padre, seguimos subiendo por el árbol de padres 
        # hasta llegar al nodo inicial
        while actual is not None:
            camino.append(actual)
            actual = padres[actual]

        # Invertimos el camino para que vaya del inicio a la meta
        camino.reverse()
